...
def analyze_speech_with_gemini(transcription_text, topic=None):
    """Analyze speech using Gemini"""
    try:
        # Define the list of filler words to search for
        filler_word_list = [
            "um", "uh", "like", "you know", "so", "actually", "basically", 
            "seriously", "literally", "well", "okay", "right", "I mean", 
            "kind of", "sort of", "stuff", "things", "anyway", "hmm", "ah", 
            "er", "mm-hmm", "uh-huh", "just", "maybe", "probably", "guess"
        ]

        analysis_prompt = f"""You are a professional speech analyst. Your task is to analyze the following speech and provide specific, numerical feedback.
...
Speaking Style
Speaking rate: 145 words per minute
Assessment: Optimal pace for comprehension
Speech duration: 180 seconds
Total duration: 210 seconds
...
Pitch
Pitch Analysis:
Pitch variation: Good variation between 150-250 Hz
Consistency: Very consistent with appropriate emphasis
Average pitch: 200 Hz

Now analyze the provided speech and provide your analysis in the same format with actual values and specific feedback. Do not copy the example - provide your own analysis based on the actual speech content."""
        ...
        # Extract pronunciation accuracy
        ...
        rate_match = re.search(r'Speaking rate: (\d+) words per minute', analysis_text)
        rate = int(rate_match.group(1)) if rate_match else 0
        
        # Extract durations
        speech_duration_match = re.search(r'Speech duration: (\d+) seconds', analysis_text)
        speech_duration = int(speech_duration_match.group(1)) if speech_duration_match else 0
        
        total_duration_match = re.search(r'Total duration: (\d+) seconds', analysis_text)
        total_duration = int(total_duration_match.group(1)) if total_duration_match else 0
        
        # Extract average pitch
        pitch_match = re.search(r'Average pitch: (\d+) Hz', analysis_text)
        average_pitch = float(pitch_match.group(1)) if pitch_match else 0.0
        
        # Initialize filler words dictionary with all possible filler words
        filler_words = {word: 0 for word in filler_word_list}
        
        # Count occurrences of each filler word in the transcription
        ...
        # Remove filler words with zero occurrences
        filler_words = {k: v for k, v in filler_words.items() if v > 0}
        
        # Calculate total filler words and percentage
        total_fillers = sum(filler_words.values())
        total_words = len(transcription_text.split())
        filler_percentage = (total_fillers / total_words * 100) if total_words > 0 else 0
        
        # Create analysis dictionary with extracted values
        analysis_dict = {
            'pronunciation': {
                'accuracy': accuracy,
            ...
            'pitch': {
                'variation': analysis_text.split('Pitch variation:')[1].split('\n')[0].strip() if 'Pitch variation:' in analysis_text else '',
                'consistency': analysis_text.split('Consistency:')[1].split('\n')[0].strip() if 'Consistency:' in analysis_text else '',
                'average': average_pitch
            },
            'speech_rate': {
                'wpm': rate,
                'assessment': analysis_text.split('Assessment:')[1].split('\n')[0].strip() if 'Assessment:' in analysis_text else '',
            ...
            'mood': {
                'primary_emotion': analysis_text.split('Primary emotion:')[1].split('\n')[0].strip() if 'Primary emotion:' in analysis_text else '',
                'formality': analysis_text.split('Formality level:')[1].split('\n')[0].strip() if 'Formality level:' in analysis_text else '',
                'audience_suitability': analysis_text.split('Audience suitability:')[1].split('\n')[0].strip() if 'Audience suitability:' in analysis_text else '',
                'mood_suitability_assessment': {
                    'assessment': analysis_text.split('Assessment:')[2].split('\n')[0].strip() if len(analysis_text.split('Assessment:')) > 2 else '',
                    'reasons': []
                }
            }
        }
...
def process_with_gemini(transcription, purpose, audience, duration, tone, additional_requirements, topic, speech_analysis):
    ...
    try:
        ...
        # Use the original transcription for both original and refined
        # since we're keeping the old refined speech generation method
...
def process_with_gpt(openai_api_key, transcription, purpose, audience, duration, tone, additional_requirements, topic, speech_analysis):
    """Process transcription and generate refined speech using GPT"""
    try:
        if not transcription or not transcription.strip():
            return "No transcription provided", "", "Please provide a valid transcription."
...
def format_detailed_feedback(results):
    ...
    if isinstance(results, str):
        # If results is a string (raw analysis), return it directly
        return f"""
    ...
    # If results is a dictionary, format it as before
        ...
        if st.button("Process Speech"):
            ...
            try:
                ...
                if uploaded_file:
                    ...
                    # Analyze speech using Gemini
                    results = analyze_speech_with_gemini(transcription, topic_of_speech)
                    if not results:
                ...
                # Step 2: Generate refined speech and detailed feedback
                status_container.markdown("""
                    <div class="stStatusContainer stInfo">
                ...
                # Generate refined speech using GPT
